...
// ==========================================
// SYSTEM PROMPT BÁCH KHOA TOÀN THƯ CHO AI CLAUDE
// ==========================================
...
2. Chốt sale thẻ SWC Pass trước deadline ${DEADLINE} (CÒN ${daysLeft} NGÀY). Khuyên mạnh Gói Plus 5 năm và Ultimate Vĩnh viễn.

THÔNG TIN KHÁCH HÀNG: Tên: ${user.firstName} ${user.lastName} | Nhóm: ${user.tag} | Gói Pass hiện tại: ${user.swcPassTier}

KIẾN THỨC NỀN TẢNG ĐỂ TƯ VẤN:
- TẦNG 5 CHUỖI THỨC ĂN: Chính phủ → Cá voi → Đội lái → Sói già → F0. F0 tự trade = tự sát, là thanh khoản cho quỹ lớn.
- BỘ LỌC CHỐNG FOMO (SWC FIELD): Không phải mò mẫm dự án rác. Mọi dự án được đóng gói pháp lý qua SPV (Phương tiện mục đích đặc biệt), bảo vệ tiền ở Mỹ/EU. ATLAS RWA Dubai thanh khoản 3s.
- KẺ HỦY DIỆT PHÍ ẨN: Quỹ khác thu phí 2% tài sản mỗi năm. SWC Pass chỉ thu phí Membership cố định ($10/tháng cho gói 5 năm). Không cắt xén lợi nhuận.
- CỖ MÁY LÃI KÉP (Road to $1M): Đầu tư $8/ngày ($240/tháng), kỷ luật 15 năm = $1,000,000. Dành 10 phút/tháng thao tác mua bán trên chính tài khoản cá nhân của khách. Minh bạch, tiền ai nấy giữ.

3 GÓI SWC PASS ĐỂ CHỐT SALE:
1. Essential (1 Năm - $240): Gói "Cà Phê Trải Nghiệm" ($20/tháng). Thử nghiệm hệ thống chuyên gia.
2. Plus (5 Năm - $600): Gói "Kỷ Luật Thép" [KHUYÊN KHÁCH MUA GÓI NÀY]. Giá cưa đôi chỉ $10/tháng (bằng 1 bát phở/tuần). Ép khách vào kỷ luật 5 năm.
3. Ultimate (Vĩnh Viễn - $2.600): Gói "Di Sản Gia Tộc". Đầu tư dài hạn, mua đứt mỏ vàng cho con cái. CHÚ Ý TỬ HUYỆT: Gói này SẼ ĐÓNG CỬA VĨNH VIỄN VÀO NGÀY ${DEADLINE}. Chỉ 1000 suất.
...
- KHÔNG nhắc tới Token, SWGT hay Airdrop. Chỉ nói về Đầu tư giá trị, SPV, Lãi kép và Hệ sinh thái SWC.`;
}

// Hàm gọi API Claude bọc thép (Xử lý lỗi luân phiên History)
async function callClaude(user, userMessage) {
    try {
        let history = user.chatHistory || [];
        history.push({ role: 'user', content: userMessage });

        // CHUẨN HÓA LỊCH SỬ CHAT: Đảm bảo luân phiên (user -> assistant -> user)
        let validHistory = [];
        let lastRole = '';
        ...
        return `Dạ hiện tại Đội ngũ chuyên gia SWC đang bận phân tích dữ liệu định lượng. Em đã báo cáo lên hệ thống, Admin sẽ trực tiếp phản hồi lại anh/chị ngay nhé! 🙏`;
    }
}
...
async function sendMainMenu(chatId, messageId = null) {
    ...
    const keyboard = {
        inline_keyboard: [
            [{ text: "🇻🇳 Cài đặt Tiếng Việt (Dành cho người mới)", url: "https://t.me/setlanguage/vi" }],
            [{ text: "📢 Kênh Tin Tức", url: `https://t.me/${CHANNEL_USERNAME.replace('@','')}` }, { text: "💬 Nhóm Giao Lưu", url: `https://t.me/${GROUP_USERNAME.replace('@','')}` }],
...
bot.on('callback_query', async (callbackQuery) => {
    ...
    // ==========================================
        ...
        text = `⚖️ <b>PHÂN TÍCH TÂM LÝ "CÁO GIÀ" ĐỂ CHỌN GÓI SWC PASS</b>\n\nChúng tôi có 3 mức vé thông hành. Hãy dùng toán học để xem cái nào thực sự có lợi cho bạn:\n\n1️⃣ <b>Gói Essential (1 Năm - $240): Gói "Cà Phê Trải Nghiệm"</b>\nDành cho người đa nghi, sợ rủi ro. 240 đô la một năm, chia ra chỉ $20/tháng – bằng đúng tiền một chầu cà phê. Thay vì uống cà phê xong là hết, 20 đô này thuê được một đội ngũ chuyên gia phân tích thị trường Mỹ. Đây là 'học phí' rẻ nhất để tránh tự trade mất hàng trăm triệu.\n\n2️⃣ <b>Gói Plus (5 Năm - $600): Gói "Kỷ Luật Thép" [🔥 KHUYÊN DÙNG]</b>\nĐầu tư tài chính nhìn ngắn 1 năm thì không bao giờ thấy sức mạnh Lãi Kép. Mua gói 5 năm, giá cưa đôi chỉ còn <b>$10/tháng</b> (tiết kiệm 50%). Cầm vé 5 năm trong tay sẽ 'ép' bạn vào một kỷ luật thép, không bỏ cuộc giữa chừng. Mọi công cụ AI mới ra mắt, bạn đều dùng miễn phí không lo nền tảng tăng giá.\n\n3️⃣ <b>Gói Ultimate (Vĩnh Viễn - $2.600): Gói "Di Sản Gia Tộc"</b>\nDành cho người có tầm nhìn xa. Đầu tư 20 năm thì mỗi năm chỉ có $130. Bạn mua ĐỨT một hệ thống tài chính để sau này chuyển giao cho con cái.\n\n⚠️ <b>CÚ CHỐT HẠ:</b> Gói Ultimate là phiên bản Launch Edition và <b>SẼ ĐÓNG CỬA VĨNH VIỄN</b> vào ngày ${DEADLINE} (Còn đúng ${daysLeft} ngày). Qua ngày này, có mang $10.000 đến cũng không thể mua đứt được nữa! Quyết định mua đứt mỏ vàng hay đi thuê mỏ vàng là ở bạn ngay lúc này!`;
        keyboard = [ NÚT_ĐĂNG_KÝ_SỰ_KIỆN, [{ text: "🔙 Quay lại SWC Pass", callback_data: 'menu_swcpass_main' }, { text: "🏠 Menu Chính", callback_data: 'main_menu' }] ];
    }
    else if (data === 'swcpass_benefits') {
        text = `🎁 <b>4 LỚP ĐẶC QUYỀN TỐI THƯỢNG CỦA SWC PASS</b>\n\n<b>1. ĐẶC QUYỀN TẠO DÒNG TIỀN (Road to $1M)</b>\nNhận tín hiệu hàng tháng (Mua mã nào, tỷ lệ bao nhiêu). Không cần phân tích chart, chỉ tốn 10 phút/tháng thao tác. Tiền vốn nằm 100% trong tài khoản chứng khoán cá nhân của bạn. Không giao tiền cho ai, không lo sập web.\n\n<b>2. SÂN CHƠI CÁ MẬP (SWC Field)</b>\nMua tài sản giá sỉ vòng Private trước khi lên sàn. Ngưỡng đầu vào siêu thấp từ $50 (thay vì $500k từ các quỹ lớn). Đầu tư minh bạch qua pháp nhân SPV (Mỹ, EU).\n\n<b>3. CỘNG ĐỒNG & SỐ HÓA</b>\nTham gia Zoom kín, Hỏi-Đáp trực tiếp chuyên gia. Hệ thống Gamification theo dõi mục tiêu cá nhân đến 1 Triệu Đô trực quan. Miễn phí nâng cấp công cụ mới trọn đời.\n\n<b>4. ĐẶC QUYỀN CHO ĐỐI TÁC HỆ THỐNG</b>\nBảo lưu cấu trúc tuyến dưới 100%. Tạo dòng tiền thụ động vĩnh cửu: Khi tuyến dưới gia hạn Membership định kỳ hàng năm để tiếp tục nhận phím lệnh, hoa hồng sẽ tự động chảy về túi bạn.`;
        keyboard = [ [{ text: "⚖️ Xem So Sánh Các Gói", callback_data: 'swcpass_compare' }], [{ text: "🔙 Quay lại", callback_data: 'menu_swcpass_main' }] ];
    }

    // ==========================================
    // NHÁNH 2: ROAD TO $1M (CẬP NHẬT FULL TEXT)
    // ==========================================
    else if (data === 'menu_road1m_main') {
        text = `🗺️ <b>HÀNH TRÌNH ĐẾN $1 TRIỆU ĐÔ</b>\n\nChiến lược do SWC Field phát triển. Đây là một chương trình đầu tư dài hạn: <b>chỉ cần đầu tư $8/ngày (khoảng $240/tháng) có kỷ luật</b>, đảm bảo đầu tư đều đặn và với <b>sức mạnh lãi kép</b>, bạn có thể hướng đến mục tiêu đạt số vốn <b>1.000.000 đô la trong 15 năm</b>.\n\nSản phẩm là một hệ thống hoàn chỉnh cho phép bạn bắt đầu đầu tư mà không cần kinh nghiệm hay các khóa đào tạo — và không cần tốn nhiều thời gian (chỉ 10-15 phút mỗi tháng).\n\n<i>Cập nhật: thg 3, 2026</i>`;
        keyboard = [
        ...
        text = `🔥 <b>MỤC TIÊU & LỢI ÍCH THỰC CHIẾN ROAD TO $1M</b>\n\n` +
               ...
               `• <b>Định hình lại thói quen (Kỷ luật thép):</b> Thay vì ném tiền vào tiêu sản (cà phê, hàng hiệu), hệ thống ép bạn kỷ luật trích ra đúng $8/ngày ($240/tháng). Biến việc đầu tư thành thói quen vô thức như đánh răng mỗi sáng.\n` +
               `• <b>Xây móng "Tự do tài chính":</b> Không phải nhìn số dư to lên để khoe khoang. Mục tiêu là dùng danh mục cổ phiếu này tạo ra Dòng tiền Cổ tức. Khi cổ tức thụ động vượt tổng chi phí sinh hoạt, bạn chính thức "nghỉ hưu" và tự do.\n` +
               `• <b>Thiết lập Di sản cho thế hệ sau:</b> Người nghèo để lại nợ nần. Người trung lưu để lại căn nhà. Người giàu thực sự để lại một Hệ thống tạo ra tiền. Tài khoản chứng khoán này chính là tài sản thừa kế vô giá, làm bệ phóng cho con cái.\n\n` +
               `<b>2. LỢI ÍCH THỰC CHIẾN: NHỮNG THỨ CHẠM ĐƯỢC BẰNG TAY</b>\n` +
               `• <b>Triệt tiêu 100% cảm xúc hoảng loạn:</b> Sát thủ lớn nhất không phải "Cá mập", mà là sự sợ hãi. Khi thị trường sập 30%, người thường cắt lỗ. Nhưng chiến lược DCA sẽ báo: "Cơ hội vàng, mua mạnh mã này!". Bạn hành động như cỗ máy vô cảm để gom tài sản giá rẻ.\n` +
               `• <b>Tiết kiệm 10.000 giờ học việc bằng máu:</b> Không cần đọc báo cáo tài chính, không phân tích nến Nhật hay MACD. Chuyên gia SWC đã làm thay. Bạn chỉ tốn 10-15 phút/tháng mở app và copy lệnh. Thời gian còn lại đi làm chuyên môn và chơi với con cái.\n` +
               `• <b>Minh bạch tuyệt đối, tự nắm đằng chuôi:</b> SWC Pass KHÔNG GIỮ TIỀN của bạn. Tiền nằm 100% trong tài khoản chứng khoán cá nhân. Chúng tôi cung cấp "bản đồ kho báu", bạn tự cầm xẻng đi đào. Cần tiền? Tự bấm bán và rút về trong 1 nốt nhạc.\n` +
               `• <b>Đa dạng hóa địa chính trị (Thoát khỏi ao làng):</b> Nếu chỉ mua BĐS trong nước, tài sản bị trói chặt vào kinh tế nội địa. Hệ thống giúp bạn rải vốn sang các thị trường quốc tế vững chãi (như chứng khoán Mỹ). Một nơi hắt hơi, nơi khác sẽ gánh vác.\n\n` +
               `<b>3. Bảo vệ khỏi cảm tính:</b> Tự động hóa việc đầu tư! Tránh FOMO đu đỉnh.`;
        ...
        text = `🔱 <b>5 TẦNG BẬC CHUỖI THỨC ĂN TÀI CHÍNH</b>\n\nBạn không nghèo đi vì thiếu thông tin, bạn nghèo vì đang chơi bằng luật của người khác.\n\n` +
               ...
               `<b>Tầng 4 — 🐺 Sói Già</b>\nTrader kỷ luật, sống sót qua nhiều chu kỳ\n→ Không trade bằng cảm xúc. Có hệ thống, có stoploss, hiểu rõ mình đang ở đâu trong chuỗi thức ăn. Số ít — rất ít.\n\n` +
               `<b>Tầng 5 — 😵 F0 / Sinh Vật Phù Du</b>\nNhà đầu tư bình thường — đám đông\n→ Mua bằng tai (nghe tin), bán bằng cảm xúc (sợ hãi). Là nguồn thanh khoản nuôi sống tất cả 4 tầng trên. 95% người tự trade đang ở đây.\n\n` +
               `💥 <b>F0 Tự trade = Tự sát.</b> SWC Pass giúp bạn nhảy thẳng lên Tầng 2, ủy thác hệ thống chuyên gia để đứng trên vai Quỹ lớn!`;
        keyboard = [
            NÚT_ĐĂNG_KÝ_SỰ_KIỆN,
    ...
    // ==========================================
        ...
        text = `🏢 <b>SWC FIELD LÀ GÌ?</b>\n\nNền tảng "Showcase" dự án đầu tư thế hệ mới. Khách hàng có thể lựa chọn rải vốn vào các dự án đã qua <b>Sơ tuyển (Đánh giá toàn diện)</b> trước khi giới thiệu.\n\nLợi thế:\n- Đầu tư từ $50 thay vì $500.000.\n- Truy cập toàn cầu: 1 tài khoản, 4 khu vực pháp lý.\n- Minh bạch tuyệt đối qua cấu trúc <b>Pháp lý SPV</b>.\n- Câu lạc bộ đầu tư: Hỗ trợ, phân tích, đào tạo.\n\n👇 <b>Hãy khám phá:</b>`;
        keyboard = [
            [{ text: "⚖️ Bộ Lọc Chống FOMO (Pháp lý SPV)", callback_data: 'swcfield_spv' }],
        ...
        text = `⚖️ <b>BỘ LỌC CHỐNG FOMO & CẤU TRÚC PHÁP LÝ SPV</b>\n\n<i>"Tôi sợ bị lừa, sợ mất tiền vào các dự án rác?"</i>\n\n<b>Bộ Lọc Chống FOMO:</b> Chúng tôi làm phần việc nhàm chán nhất để bảo vệ bạn khỏi những quyết định đầu tư cảm xúc. Bạn không phải đoán xem dự án nào an toàn.\n\nThay vì bạn mua một "cổ phần trừu tượng" mơ hồ, mỗi dự án trên SWC Field đều được đóng gói thành một <b>SPV (Phương tiện mục đích đặc biệt)</b> riêng biệt.\n\nBạn mua cổ phiếu của SPV đó, mang lại cho bạn cổ phần được công nhận hợp pháp trong nhóm tài sản tổng thể. Tất cả giao dịch được thực hiện qua các nền tảng quản lý nghiêm ngặt tại Mỹ, EU, Nga. Tiền của bạn được bảo vệ như cách giới siêu giàu đang làm!`;
        keyboard = [[{ text: "🔙 Quay lại SWC Field", callback_data: 'menu_swcfield_main' }]];
    }
    else if (data === 'swcfield_atlas') {
        text = `🏢 <b>SIÊU DỰ ÁN ATLAS — BĐS SỐ HÓA DUBAI (RWA)</b>\n\nSở hữu và giao dịch <b>Bất động sản Dubai</b> chỉ bằng vài cú chạm trên điện thoại.\n\n🌟 <b>Điểm nổi bật:</b>\n• <b>Thanh khoản 3 giây</b> — phá vỡ sự chậm chạp BĐS truyền thống.\n• <b>Pháp nhân Atlas Overseas FZE</b> — cấp phép bởi Trung tâm Thương mại Dubai.\n• <b>Đầu tư từ $50</b> — Dân chủ hóa sân chơi của giới siêu giàu.\n• <b>RWA (Real World Assets)</b> — Tài sản thật, tạo ra dòng tiền thật.\n\n⚠️ <b>LƯU Ý KHẨN:</b> Vòng ưu đãi đóng cửa vào <b>${DEADLINE}</b>. Đừng bỏ lỡ vị thế sớm nhất!`;
        keyboard = [ NÚT_ĐĂNG_KÝ_SỰ_KIỆN, [{ text: "🔙 Quay lại SWC Field", callback_data: 'menu_swcfield_main' }] ];
    }
    ...
    // ==========================================
        ...
        text = `✅ <b>KẺ HỦY DIỆT PHÍ ẨN</b>\n\n<i>"Tôi ghét bị ngân hàng và các quỹ cắn xén tiền lãi bằng các loại phí vô hình."</i>\n\nNếu đưa tiền cho các quỹ truyền thống, họ sẽ cắt xén 1.5% - 2% trên TỔNG tài sản của bạn mỗi năm (Phí này cướp đi hàng tỷ đồng sau 20 năm).\nỞ SWC, chúng tôi chơi công bằng: <b>Mô hình Thẻ Thành Viên (Membership)</b>. Bạn chỉ trả đúng $10/tháng (gói 5 năm). Dù tài sản tăng lên 1 triệu đô, phí vẫn chỉ là $10. Chúng tôi không phạt bạn vì bạn giàu lên!`;
        keyboard = [[{ text: "🔙 Quay lại FAQ", callback_data: 'faq_back' }]];
    }
...
bot.onText(/\/(admin|menu)/i, async (msg) => {
    ...
    bot.sendMessage(msg.chat.id, `👨‍💻 <b>ADMIN PANEL — SWC BOT v5.0 (Bản Master Chốt Sale)</b>`, { parse_mode: 'HTML', reply_markup: { inline_keyboard: [[{ text: "📊 Thống kê Phễu Khách Hàng", callback_data: 'admin_stats' }], [{ text: "📢 Bảng lệnh Quản trị", callback_data: 'admin_help' }]] } });
});
