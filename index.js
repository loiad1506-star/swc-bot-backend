    ...
    // --- API NHẬN THƯỞNG MỐC + BÁO CÁO GROUP ---
        ...
        req.on('end', async () => {
            try {
                ...
                // Thêm tính năng bỏ qua thời gian khóa nếu đạt 1500 SWGT
        ...
        if (status.error) { bot.answerCallbackQuery(callbackQuery.id, { text: "⚠️ Bot chưa được cấp quyền Admin trong Nhóm/Kênh!", show_alert: true }); } 
            ...
            if (user.groupMessageCount < 1) {
                ...
                if (!user.task1Done) {
                    ...
                    // Gửi tin nhắn thưởng Task 1 kèm thông báo chuẩn bị chuyển bước
                    ...
                    // AUTO FLOW: Tự động chuyển Task 2 sau 2 giây
                    setTimeout(() => {
