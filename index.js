...
const server = http.createServer(async (req, res) => {
    ...
    if (parsedUrl.pathname === '/api/user' && req.method === 'GET') {
        ...
        if (!userData) {
            userData = { balance: 0, wallet: '', gatecode: '', fullName: '', email: '', phone: '', referralCount: 0, isPremium: false, joinDate: Date.now() };
        }
        
        // KIỂM TRA ĐIỂM DANH HÔM NAY (Cho Frontend vô hiệu hoá nút)
        let isCheckedInToday = false;
        if (userData.lastCheckInDate) {
            const now = new Date();
            const last = new Date(userData.lastCheckInDate);
        ...
        res.end(JSON.stringify({ ...userData._doc, isCheckedInToday }));
    } 
    ...
    // API NHẬP MÃ GIFTCODE
        ...
        req.on('end', async () => {
            try {
                const data = JSON.parse(body);
                const inputCode = data.code ? data.code.trim().toUpperCase() : ""; // Bỏ khoảng trắng, viết hoa
        ...
        req.on('end', async () => {
            try {
                ...
                if (lastCheckin) {
                    lastCheckinDay = new Date(lastCheckin.getFullYear(), lastCheckin.getMonth(), lastCheckin.getDate());
                }
                ...
                // CẬP NHẬT: Tính khoảng cách ngày chính xác để không xoá chuỗi của người hôm qua
                if (lastCheckinDay) {
                    const diffTime = today.getTime() - lastCheckinDay.getTime();
                    const diffDays = Math.round(diffTime / (1000 * 60 * 60 * 24)); // Tính tròn ngày
                ...
                res.writeHead(200, { 'Content-Type': 'application/json' });
                res.end(JSON.stringify({ success: true, balance: user.balance, reward: reward, streak: user.checkInStreak, isCheckedInToday: true }));
            } catch (e) { res.writeHead(400); res.end(); }
        });
    }
    ...
    // CẬP NHẬT: Thêm hiển thị Bảng Vàng (Cấp Bậc & Huy hiệu)
